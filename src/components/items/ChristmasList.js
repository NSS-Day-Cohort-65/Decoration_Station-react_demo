...
export const ChristmasItems = () => {
  const [christmas, setChristmas] = useState([]) // returns an array: [stateVariable, setStatefunction] takes one argument: the initial value of the state variable
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8088/items?seasonId=2`)
      .then((res) => res.json())
      .then((itemsArray) => {
        setChristmas(itemsArray)
      })
